...
# Ensure workspace root is importable so `firewall.*` and `analysers.*` resolve.
_WORKSPACE_ROOT = Path(__file__).resolve().parent.parent
if str(_WORKSPACE_ROOT) not in sys.path:
    sys.path.insert(0, str(_WORKSPACE_ROOT))

from firewall.core.agent import AgenticBrowser
from firewall.core.agent_firewall import AgentFirewall
